...
/*
        //Leaves stuff
        const redirJson = await loadRedirectJson(); 
        for( let redir of redirJson.redirs )
        {
            const redirPath = redir + "/user/mods/" + projectName;
            logger.log("success","LEAVES: Pushing to: " + redirPath );
...
async function loadRedirectJson() {
    const JsonPath = "../../../Leaves Mods/redirect.json";
